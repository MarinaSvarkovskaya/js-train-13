...
function manipulateUrl(url) {
  ...
  // let a = url.protocol;
  // let b = url.host;
  // let c = url.searchParams.append(`newParam`, `newValue`);
  // let d = url.searchParams.delete(`oldParam`);
  // return { a, b, c, d };
}
...
function searchParamsURL(url) {
  ...
  // Повертаємо словник
  let a = new URL(url);
  let URLSearchParams = a.searchParams;
  let params = new Map();
  for (const [b, value] of URLSearchParams) {
    params.set(b, value);
  }
  return params;
}
...
function processUrl(url, options) {
  ...
  for (const key in options) {
    ...
    if (option.protocol) {
      urlObj.searchParams.set(key, options.protocol);
    }
    if (urlObj.host) {
      urlObj.searchParams.set(key, options.host);
    }
  }
  return urlObj.toString();
}
